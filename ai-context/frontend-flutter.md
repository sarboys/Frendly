...
- Chats tab mirrors `front/src/pages/v5/Chats.tsx`: editorial header with only search action, search launcher, active rail, `Все / Клубы / Встречи / Дейтинг / Личные` chips, mixed all-chat list with joined community chats, terracotta unread counters, v5 fallback rows when backend chats are empty, and terracotta AI launch CTA.

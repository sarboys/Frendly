...
Unified meetup evening flow lives at `AppRoute.eveningFlow` (`/evening/:eventId`) in `features/evening_flow/presentation/evening_flow_screen.dart`. Meetup detail keeps the chat icon, but the primary joined CTA is `Начать вечер`, and direct join opens this flow after existing `joinEvent`. The flow uses existing production APIs only: `eventDetailProvider`, `checkInProvider`, `liveMeetupProvider`, `afterPartyProvider`, `confirmCheckIn` and `saveAfterParty`. Do not add static after-party vote venues until backend returns real vote options.

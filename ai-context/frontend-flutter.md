...
- Match lives at `AppRoute.match` (`/match/:userId`). Dating reciprocal likes from discover push this fullscreen match moment; incoming likes still keep the existing one-tap matched chat pill. The match screen reads `matchesProvider`, renders the v5 warm fullscreen avatar-convergence UI, opens direct chat through `createOrGetDirectChat`, and opens Create Meetup with `inviteeUserId`.

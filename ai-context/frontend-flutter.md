...
- Chats tab mirrors `front/src/pages/v5/Chats.tsx` and the `/v5/chats-alt` update: editorial header with only search action, search launcher, active rail, mini radar `Сейчас`, icebreaker chips, `Все / Клубы / Встречи / Дейтинг / Личные` chips, mixed all-chat list with joined community chats, terracotta unread counters, v5 fallback rows when backend chats are empty, and a fixed bottom AI compass button with microphone that opens AI Voice.

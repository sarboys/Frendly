...
- `LocalFirstRepository` wraps cache plus network fetch: cached data can return immediately, refresh runs in the background, network errors are preserved when cache is empty.
- `ChatLocalStore` stores raw chat summaries, recent messages and sync cursors. Message upsert merges by `messageId` and `clientMessageId`, so server ack can replace a local pending row.
...
- Own profile at `/profile` mirrors `front/src/pages/v5/Profile.tsx`: V5 header, hero, social counters, quick entries, intent, vibe, interests, about and history. Do not add the old lower action cards back to this screen.

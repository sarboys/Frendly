...
- Chats keep the existing pin icon and add chat-row long press plus left swipe delete for meetup, direct and club chat rows. Meetup delete asks for confirmation and means leaving the event or evening session for non-host users. Direct delete hides the chat for the current user. Club delete asks for confirmation and means leaving the club for non-owner users. Delete is optimistic against the local chat-list state and rolls back on API error.

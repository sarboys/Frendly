...
- AI Create lives at `AppRoute.aiCreate` (`/ai-create`) and mirrors `front/src/pages/v5/AICreate.tsx`: prompt-only input, prompt guidance, examples and a mic entry into the voice flow. Mobile sends only `prompt` plus saved manual city/coordinates to `POST /evening/routes/ai-drafts`; backend LLM intent infers roles, budget and step count. Empty prompt does not call the backend. The generated draft displays the active review step first, lets the user accept or regenerate one step, and keeps `Создать встречу` plus manual edit disabled until all steps are accepted. The refresh icon inside the generated plan uses `POST /evening/routes/ai-drafts/:draftId/regenerate`, not a new create request, so the backend can reject current step ids and return a different route. Confirm calls `POST /evening/routes/ai-drafts/:draftId/confirm`, then opens the common `PublishMeetupScreen`.

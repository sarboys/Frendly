...
- Session cleanup clears user-scoped local cache, chat rows, sync cursors and pending commands only when the cache store is already active, so current network-first tests and startup paths do not open Drift just to clear it.

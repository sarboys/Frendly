...
- Chats keep the existing pin icon and add chat-row long press plus left swipe delete for meetup and direct chats. Meetup delete asks for confirmation and means leaving the event for non-host users. Direct delete hides the chat for the current user. Club chats are not part of this delete flow. Delete is optimistic against the local chat-list state and rolls back on API error.

...
- Local-first cache foundation lives in `mobile/lib/app/core/local_cache/`.
...
- Local-first is gated by `BIG_BREAK_LOCAL_FIRST_CACHE`; default is enabled in every build mode, including release. Use `--dart-define=BIG_BREAK_LOCAL_FIRST_CACHE=false` only as a manual kill switch. `localFirstCacheEnabledProvider` can disable providers, and DB open failure sets `appLocalCacheRuntimeDisabledProvider`.
...
- Host dashboard at `/host` mirrors `front/src/pages/v5/Host.tsx`: V5 hero metric, stat cards, create CTA, real join requests with approve/reject actions, and tabs for upcoming, past and draft meetups backed by `/host/dashboard`. For live or past hosted meetups, the dashboard opens a finish sheet backed by `GET /host/events/:eventId`; the host selects only people who were actually present, then mobile sends those ids to `POST /host/events/:eventId/live/finish`.
...
- Meetup event summaries can include `imageUrl` from linked Affiche content. Backend-owned Affiche images normally arrive as CDN URLs, while legacy relative proxy paths still resolve through `resolveBackendUrl` before the UI falls back to emoji gradients.
...
- Create Meetup route picker mirrors `front/src/pages/v5/CreateMeetup.tsx`: one v5 bottom sheet with `Маршруты вечера`, search, `Создать свой маршрут` CTA into the local route constructor, and compact route rows backed by `eveningRouteTemplatesProvider`. Do not bring back the old `Готовые` and `Свой` tabs.
...
- Settings mirrors the v5 settings-alt concept: profile tile, Frendly+/Wallet, verification/SOS, evening and search entries, privacy, typed notification rows, account and danger zone. Do not show dark theme or appearance controls. Keep backend-backed settings on existing `UserSettingsData` fields and do not show internal testing access toggles on this user-facing screen.
...
- Meetup detail at `AppRoute.eventDetail` (`/event/:eventId`) mirrors `front/src/pages/v5/MeetupDetail.tsx`: compact emoji hero with status and seat pills, thin time/place/going meta row, slim verified host row, host quote, interactive route block with mini map and numbered pins. If the backend event has one place, the block title is `Место`; route-backed events use existing evening route data and show `Маршрут вечера` with stop rows. Attendees render as a compact avatar stack, partner perk and Safe Walk are slim cards, and sticky bottom CTA stays. Join, request, leave, invite, host edit, chat, tickets and booking still use existing backend flows. Pending join requests disable the primary CTA as `Заявка отправлена` and keep cancel available.
...
- Payments use backend catalog and server-side T-Bank init/check only for token packs. Flutter opens only token `paymentUrl`; `frendly://payment/...` routes through `/payment/:result`, checks the order, then refreshes the token wallet.
...
- `paymentCatalogProvider` reads `/payments/catalog` and exposes token packs, promo options and Frendly+ token costs. Paywall and token packs should not hardcode prices except as fallback.

...
User-facing AI drafts run a fast Qwen intent pass before building the candidate pack. That intent pass returns ordered roles and per-step search hints, so arbitrary prompts and repeated role types are supported without adding one-off rules. Source mapping stays backend-owned: Tomesto for food, bars, clubs and restaurants; Ticketland/MTS Live for theatre, shows, concerts and standup; KudaGo for walks, parks, free activities and city events. Walk roles use an extra semantic filter after DB search, so `Каток у парка` is not treated as a walk just because it mentions a park.

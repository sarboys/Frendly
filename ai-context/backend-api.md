...
- Event list and detail summaries expose radar category fields: `routeId` from `eveningRouteId`, `routePointCount` from route steps count, and `isAfficheBacked` for events created from public Affiche content. Mobile uses them with ticket source fields to calculate Radar counts for meetings, routes and affiche without extra requests.

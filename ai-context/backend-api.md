...
- User-facing AI route resolve first tries to create a draft `EveningRoute` from imported external content when `prompt` is present. Establishments come from Tomesto, concerts/theatre/standup/show steps from `advcake_ticketland` Ticketland/MTS Live, and free walks/parks/activities from KudaGo. Each next step must be near the previous one; old curated route matching remains the fallback when no nearby external route can be built.

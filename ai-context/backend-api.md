...
- AI draft intent supports exact place constraints through `exactPlaceNames` and `exactAddress`. If a requested exact place has no matching candidate in the selected city/source, backend returns `404 evening_ai_exact_place_not_available` instead of silently substituting another venue.
- AI draft intent exposes specific KudaGo-backed roles `activity`, `culture`, `movie`, `viewpoint`, `shopping` and `wellness`; legacy `free_activity` remains accepted for stored/fallback intent, but normal prompt rules prefer the specific roles.

...
- External content import mirrors imported image URLs to S3 under `external-content/...` and stores the resulting public asset URL back in `ExternalContentItem.imageUrl` when mirroring succeeds. Public Affiche API rewrites owned mirrored URLs to `/affiche/images?key=...`, then redirects through a short-lived signed S3 download so imported images do not depend on public bucket ACL or CDN DNS. If image download or S3 write fails, import keeps the source image URL and does not fail the run.

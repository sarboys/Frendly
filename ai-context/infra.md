...
- Tomesto rollout is opt-in and Moscow-only. Use `CONTENT_IMPORT_SOURCES=tomesto`, `CONTENT_IMPORT_CITIES=Москва`, `TOMESTO_REF_QUERY=...`, `TOMESTO_REQUEST_DELAY_MS=500`, `TOMESTO_MAX_PAGES=40`, `TOMESTO_WINDOW_DAYS=30`, `TOMESTO_IMPORT_IMAGES=false`, `TOMESTO_PUBLIC_EVENTS_ENABLED=false`. Production compose passes these Tomesto env names and gives content import a 30 minute default timeout, so manual imports do not get stuck on the full Moscow catalog. Production schedule stays off until permission or API is confirmed. Manual admin import is the safe first path. Places refresh without a date period; events and promos use the Tomesto date window. Promos import hidden until a promo surface exists. Expected logs: INFO for Moscow import start, created runs, discovered counts and hidden counts; WARN for unsupported city or missing ref query; ERROR for non-2xx Tomesto pages.
